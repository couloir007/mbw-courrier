  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($result) {
      case SAVED_NEW:
        ...
        $this->logger('mbw_address')->notice('Created new address %label', $logger_arguments);
        break;

      case SAVED_UPDATED:
        $this->messenger()->addStatus($this->t('The address %label has been updated.', $message_arguments));
        $this->logger('mbw_address')->notice('Updated address %label.', $logger_arguments);
        break;
    }

    $form_state->setRedirect('entity.address.collection');
